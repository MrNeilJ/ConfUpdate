...
function AutoLogin {
	Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' -Name DefaultUserName | Out-File C:\AutoLogincCheck.log
	if((Get-Content "C:\AutologinCheck.log") -eq $Null){
		Set-Itemproperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' -Name AutoAdminLogon -value '1'
		New-Item –Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" –Name DefaultUserName
		Set-Itemproperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' -Name DefaultUserName -value "THETRADEDESK\\conference"
		New-Item –Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" –Name DefaultPassword
...
function Schedule-Setup {
	ipmo ScheduledTasks
	$action = New-ScheduledTaskAction -Execute 'Powershell.exe' -Argument "-NoProfile -command Start-Process powershell -Verb runas -ArgumentList $PSScriptRoot"
	$trigger = New-ScheduledTaskTrigger -AtLogOn
	Register-ScheduledTask -Action $action -Trigger $trigger -TaskName 'ConfUpdate' -Description 'Setup computer profile' -WarningAction SilentlyContinue -ErrorAction SilentlyContinue -ErrorVariable RenSchTask -Verbose:$false | out-null
}

function Meraki-Setup {
...
function WiFi-Setup {
	...
	Invoke-Expression WaitForKey
	Get-NetAdapter -Name "Wi-Fi" | Out-File C:\WifiStatus.log
	if((Get-Content "C:\WifiStatus.log") -eq $Null){
		Disable-NetAdapter -Name "Wi-Fi" -Confirm:$false
	}
	else {
		Disable-NetAdapter -Name "Wi-Fi2" -Confirm:$false
	}
	Remove-Item C:\WifiStatus.log
}
...
function Conf-Setup {
	...
	Disable-ScheduledTask -TaskName "ConfUpdate"
...
# CONF PC SETUP
# ----------------------------------------------
# Description: Sets the entire Confrerence PC
# up for actual deployment
#
# Created By: Neil J
################################################
$FileName = "C:\PSWindowsUpdate.log"
if (Test-Path $FileName) 
{
  Remove-Item $FileName
}

# First Run through (wont have the PSWindowsUpdate Installer so we know it's a first run)
...
# Check to see if there were any more updates to do
if((Get-Content "C:\PSWindowsUpdate.log") -eq $Null){
		Write-Host "No more updates generated"
		Remove-Item C:\PSWindowsUpdate.log
		Get-InstalledModule -Name "PSWindowsUpdate" | Uninstall-Module
		
		# Complete the rest of the setup of the system
		Conf-Setup
